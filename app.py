...
def generate_single_video(
    ...
    # ) 
        ...
        try :
            image_prompt_generation_prompt = f"Write a video from for a shot of someone that shows off {video_topic}. So he's like showing it off to the camera recommending it for 5 seconds. Describe Only what you seen by a camera. no speech, trying to tell to people thru  directly! .make it look candid like user gen content. pick the charachter showing off to be appropriate to the topic. keep the top third of the frame for text as Negative space.Make the video loop seamlessly, with smooth motion that ends in the same position or state it started ins  "
